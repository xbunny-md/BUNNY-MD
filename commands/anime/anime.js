...
const ANIME_APIS = [
  ...
  // 9. AnimeNewsNetwork
  async (query) => {
    const res = await axios.get(`https://cdn.animenewsnetwork.com/encyclopedia/api.xml`, {
      params: { anime: query },
      timeout: TIMEOUT
    })
    return null // XML parsing needed - skip for now
  },
  ...
  // 13. AnimePlanet
  async (query) => {
    const res = await axios.get(`https://www.anime-planet.com/anime/all`, {
      params: { name: query },
      timeout: TIMEOUT,
      headers: { 'User-Agent': 'Mozilla/5.0' }
    })
    return null // HTML scraping needed - skip
  },
...
export default async function anime(sock, { msg, from, args }, botSettings) {
  ...
  try {
    ...
    for (let i = 0; i < ANIME_APIS.length; i++) {
      try {
        console.log(`[ANIME] Trying API ${i + 1}/${ANIME_APIS.length}`)
        animeData = await ANIME_APIS[i](query)
        if (animeData && animeData.title) {
          apiUsed = i + 1
          console.log(`[ANIME] API ${i + 1} SUCCESS`)
          break
        }
      } catch (e) {
        console.log(`[ANIME] API ${i + 1} failed: ${e.message}`)
      }
    }
...
│
│ Synopsis: ${(animeData.synopsis || 'No synopsis').slice(0, 200)}...
╰⊷ *Source: API ${apiUsed} • Powered By Bunny Tech*`
    ...
    let errorMsg = '> Anime search failed'
