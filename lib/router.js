...
const cache = require('./cache'); // Tunatumia cache engine yetu mpya
...
async function routeMessagePipeline(sock, chatUpdate, serverId, botId) {
    try {
        ...
        // Dynamic Config: Tunatumia Cache kwanza kabla ya Database
        ...
        if (!liveConfig) {
            ...
            liveConfig = data;
            cache.set(`config:${botId}`, data, 60000); // Caching for 1 minute
        }
        ...
        if (fs.existsSync(baseObserversDirectory)) {
            const observerFiles = fs.readdirSync(baseObserversDirectory).filter(f => f.endsWith('.js'));
            for (const file of observerFiles) {
                try {
                    const observer = require(path.join(baseObserversDirectory, file));
        ...
        // Core Management Commands (Dynamic & Editable)
        ...
        if (managementCommands[targetedCommandTrigger]) {
            if (!isInfrastructureOwner) return await sock.sendMessage(remoteJid, { text: "Access Denied." });
            ...
            cache.invalidate(`config:${botId}`); // Clear cache on update
            return await sock.sendMessage(remoteJid, { text: `Success: ${updateField} updated.` });
        }

        // Lazy Loading Commands Architecture
        const baseCommandsDirectory = path.join(__dirname, '..', 'commands');
        let resolved = false;

        if (fs.existsSync(baseCommandsDirectory)) {
            const groups = fs.readdirSync(baseCommandsDirectory).filter(f => fs.statSync(path.join(baseCommandsDirectory, f)).isDirectory());
            for (const group of groups) {
                const cmdPath = path.join(baseCommandsDirectory, group, `${targetedCommandTrigger}.js`);
                if (fs.existsSync(cmdPath)) {
                    delete require.cache[require.resolve(cmdPath)]; // Force refresh
        ...
        // Fallbacks
        if (!resolved && (targetedCommandTrigger === 'ping' || targetedCommandTrigger === 'menu')) {
            await sock.sendMessage(remoteJid, {
                text: `╭─⌈ *${liveConfig.bot_name}* ⌋\n│\n│ Status: Online\n│ Prefix: ${systemPrefix}\n╰⊷ ${liveConfig.bot_footer}`,
                contextInfo: { externalAdReply: { title: liveConfig.bot_name, body: liveConfig.bot_footer, thumbnailURL: liveConfig.bot_pic, sourceUrl: 'https://bunny-bot.mooo.com/pair' } }
            });
        }
