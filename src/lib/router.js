  ...
  for (const item of items) {
    ...
    if (stat.isDirectory()) {
      ...
      try {
        ...
        // ✅ DUPLICATE COMMAND CHECK - INAZUIA OVERWRITE
...
// 5. LID TO JID RESOLVER - INABADILISHA LID KUWA NUMBER HALISI
function resolveLid(participants, lid) {
  if (!lid ||!lid.endsWith('@lid')) return lid
  const found = participants.find(p => p.id === lid)
  return found?.jid || lid // Kama haijapatikana, rudisha LID yenyewe
}
...
export async function handleMessages(sock, m, botSettings) {
  try {
    ...
    // ✅ ADMIN CHECK + GROUP METADATA - FIX YA TAGALL
    let isAdmin = false
    ...
    if (isGroup) {
      try {
        ...
        // ✅ RESOLVE SENDER LID TO JID - FIX YA GETPP, KICK, PROMOTE
    ...
    // ✅ RESOLVE MENTIONS & REPLY LIDS - INASAIDIA COMMANDS 50+
    ...
    if (contextInfo) {
      ...
      // Resolve replied participant - getpp uki-reply
      if (contextInfo.participant) {
        contextInfo.participant = resolveLid(participants, contextInfo.participant)
      }
    }

    // ✅ RUN OBSERVERS - RUHUSA ZOTE: antidelete, autoview, autoreact, antilink, antispam, antitag, antitoxic, anticall, antiedit, goodbye, welcome
    for (const observer of observers) {
      try {
        await observer.run(sock, {
          msg, // msg sasa ina mentionedJid na participant zilizobadilishwa kuwa JID
          from,
          sender, // Sasa ni JID halisi sio LID
          body,
          isGroup,
          isStatus, // Kwa autoview status
          pushName,
          isAdmin, // Kwa antilink, antispam
          isBotAdmin, // Kwa antidelete, kick
          groupMetadata // Kwa data zote za group
        }, botSettings)
      } catch (err) {
    ...
    if (commands.has(providedName)) {
      ...
      // ✅ TUMETUMA isAdmin na isBotAdmin - FIX YA TAGALL, REVOKE, HIDETAG
