...
async function loadCommands(dir) {
  ...
  for (const item of items) {
    ...
    if (stat.isDirectory()) {
      ...
      try {
        // Weka delay 50ms kati ya kila file kama unataka
        ...
        // HAPA NDIPO PENYE SIRI - LOG ERROR TU, USI-CRASH
...
// 2. LOAD ALL OBSERVERS - These run on every message without prefix
async function loadObservers() {
  ...
  try {
    ...
    for (const file of observerFiles) {
      try {
        await new Promise(resolve => setTimeout(resolve, 50)) // Delay kidogo
...
// 3. INIT FUNCTION - CALL THIS FROM index.js
